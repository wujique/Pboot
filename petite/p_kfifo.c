...
/* 求可写入元素数 */
int pkfifo_empty(struct _pkfifo *fifo)
{
	int fsize;

	if(fifo->in >= fifo->out) fsize = fifo->size - fifo->in + fifo->out;
	else fsize = fifo->out - fifo->in;
	
	return fsize;

}
/* 缓冲有效数据 */
int pkfifo_len(struct _pkfifo *fifo)
{
	int tmp;
	
	tmp = pkfifo_empty(fifo);

	return fifo->size - tmp;
}
...
int pkfifo_out(struct _pkfifo *fifo, void *buffer, unsigned int len)
{
	...
	while(1) {
		...
		rp = (char *)fifo->data;
		rp += fifo->out*fifo->esize;
		
		for(i = 0; i< fifo->esize;i++) {
			*wp = *(rp+i);
			wp++;
		}

		fifo->out++;
		if(fifo->out >= fifo->size) fifo->out = 0;
...
/* 清空缓冲*/
int pkfifo_clear(struct _pkfifo *fifo)
{
	fifo->out == fifo->in;	
	return 0;
}
